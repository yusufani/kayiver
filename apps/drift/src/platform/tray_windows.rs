...
//! input hooks — if anything here fails, drift keeps working. Shows drift's
...
fn tooltip() -> String {
    ...
    match STATE.load(Ordering::Relaxed) {
        2 => format!("drift — controlling this PC (from {host})"),
        1 => format!("drift — connected to {host}"),
        _ => "drift — waiting for host…".to_string(),
    }
}
...
unsafe fn refresh(show_balloon: bool) {
    ...
    if show_balloon {
        ...
        fill(&mut data.szInfoTitle, "drift connected");
...
unsafe fn run() {
    ...
    let class_name = to_wide("drift_tray_class");
    let wc = WNDCLASSW {
