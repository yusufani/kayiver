...
- **Capture**: `CGEventTap` at the HID level on a dedicated run-loop thread.
  macOS auto-disables taps it deems slow; drift re-enables inside the
...
- **Permissions**: Accessibility (active tap + posting) and Input Monitoring
  (listening). Granted to whatever launches drift (your terminal during
  testing, the binary itself once autostarted). `drift doctor` reports both.
...
- **Capture**: `WH_MOUSE_LL` + `WH_KEYBOARD_LL` on a thread with a message
  ...
  eat outward motion).
- **Injected-event loopback**: everything drift injects carries
...
- **Permissions**: none. (If you ever run an elevated app, Windows UIPI will
  block injection into it unless drift is also elevated — known limitation.)
...
- Rust core (`drift-core` compiles unchanged for `aarch64-linux-android`)
...
a drift *client* (receive your mouse) without jailbreaking, period. What is
...
input button, or tools like Lunar/ControlMyMonitor alongside drift.
