...
pub enum Role {
    /// The machine that displays the code (`kayiver pair`).
    Display,
    /// The machine where the user types the code (`kayiver join`).
    Input,
}

/// Generate a human-friendly 6-digit pairing code.
/// Short codes are safe here because SPAKE2 limits attackers to one online
/// guess per connection attempt and `kayiver pair` accepts a single attempt.
...
fn confirm_tag(key: &[u8], role: Role) -> [u8; 32] {
    let tag: &[u8] = match role {
        Role::Display => b"kayiver-pair-confirm-display",
        Role::Input => b"kayiver-pair-confirm-input",
    };
...
fn derive_psk(key: &[u8]) -> [u8; 32] {
    let mut h = Sha256::new();
    h.update(key);
    h.update(b"kayiver-session-psk-v1");
    h.finalize().into()
}
...
pub async fn exchange(
    ...
    let (state, outbound) = Spake2::<Ed25519Group>::start_symmetric(
        ...
        &Identity::new(b"kayiver-kvm-pairing-v1"),
    );
